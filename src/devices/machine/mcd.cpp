...
DEFINE_DEVICE_TYPE(MCD, mcd_device, "mcd_isa", "Mitsumi CD-ROM")

//**************************************************************************
//  LIVE DEVICE
//**************************************************************************

void mcd_device::device_add_mconfig(machine_config &config)
{
...
//-------------------------------------------------
//  mcd_device - constructor
//-------------------------------------------------

mcd_device::mcd_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
...
bool mcd_device::read_sector(bool first)
{
...
uint16_t mcd_device::dack16_r(int line)
{
	if(m_buf_count & ~1)
	{
		uint16_t ret = 0;
...
void mcd_device::reset_w(uint8_t data)
{
	reset();
}

void mcd_device::cmd_w(uint8_t data)
{
	if(m_cmdrd_count)
	{
		m_cmdrd_count--;
